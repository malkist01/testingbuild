...
# Clone Neutron Clang Toolchain
if [ ! -d "$HOME/neutron-clang" ]; then
    if ! git clone https://gitlab.com/AzyrRuthless/neutron.git --depth=1 "$HOME/neutron-clang"; then
      tg "❌ Failed to clone Neutron Clang Toolchain!"
      exit 1
    fi
fi
...
# Prioritize Neutron Clang in PATH
export PATH="$HOME/neutron-clang/bin:$PATH"
export ARCH=arm64
...
echo "ld.lld version in Neutron Clang toolchain:"
if [ -f "$HOME/neutron-clang/bin/ld.lld" ]; then
  "$HOME/neutron-clang/bin/ld.lld" --version
else
  echo "ld.lld not found in Neutron Clang toolchain."
fi
echo "=============================================="
# --- End Verification ---
