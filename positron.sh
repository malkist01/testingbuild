...
if [ -d "$GITHUB_WORKSPACE/Positron_Kernel" ]; then
  log "🗑️ Cleaning up previous kernel directory..."
  rm -rf "$GITHUB_WORKSPACE/Positron_Kernel"
fi

# --- Clone the kernel source ---
log "⬇️ Cloning kernel source from: $KERNEL_SOURCE (branch: $KERNEL_BRANCH)..."
if ! git clone "$KERNEL_SOURCE" -b "$KERNEL_BRANCH" "$GITHUB_WORKSPACE/Positron_Kernel" --depth=1; then
  handle_error "Failed to clone kernel source"
fi
cd "$GITHUB_WORKSPACE/Positron_Kernel" || handle_error "Failed to enter kernel directory"

# --- Integrate KernelSU-Next ---
log "🧩 Integrating KernelSU-Next..."
KERNELSU_DIR="$GITHUB_WORKSPACE/Positron_Kernel/kernel/KernelSU-Next"
...
cd "$GITHUB_WORKSPACE/Positron_Kernel"

# --- Determine driver directory ---
if [ -d "$GITHUB_WORKSPACE/Positron_Kernel/common/drivers" ]; then
  DRIVER_DIR="$GITHUB_WORKSPACE/Positron_Kernel/common/drivers"
elif [ -d "$GITHUB_WORKSPACE/Positron_Kernel/drivers" ]; then
  DRIVER_DIR="$GITHUB_WORKSPACE/Positron_Kernel/drivers"
else
...
if [ ! -d "$HOME/Zyc-Clang" ]; then
  ...
  wget -q --progress=dot "$LATEST_RELEASE_URL" -O "$HOME/Zyc-Clang.tar.gz" 2>&1 | 
    awk '/[0-9]%/ {printf "\r📥 Downloading... %s", $NF} END {print "\r📥 Downloading... ✅"}'

  printf "📂 Extracting... "
  mkdir -p "$HOME/Zyc-Clang"
  tar -xf "$HOME/Zyc-Clang.tar.gz" -C "$HOME/Zyc-Clang" && printf "\r📂 Extracting... ✅\n"
  rm "$HOME/Zyc-Clang.tar.gz"

  log "✅ Zyc-Clang is ready at $HOME/Zyc-Clang"
else
...
make -j$(nproc --all) O=out ARCH=arm64 CC=clang LLVM=1 LLVM_IAS=1 \
    LD=ld.lld CROSS_COMPILE=aarch64-linux-gnu- CROSS_COMPILE_ARM32=arm-linux-gnueabihf- \
    2>&1 | tee build.log
...
if ! git clone -q https://github.com/AzyrRuthless/AnyKernel3 "$GITHUB_WORKSPACE/Positron_Kernel/anykernel"; then
  handle_error "Failed to clone AnyKernel3"
fi

# --- Copy files to AnyKernel3 ---
log "➡️ Copying Image.gz..."
cp "$GITHUB_WORKSPACE/Positron_Kernel/out/arch/arm64/boot/Image.gz" "$GITHUB_WORKSPACE/Positron_Kernel/anykernel"
log "➡️ Copying dtbo.img..."
cp "$GITHUB_WORKSPACE/Positron_Kernel/out/arch/arm64/boot/dtbo.img" "$GITHUB_WORKSPACE/Positron_Kernel/anykernel"
log "📁 Creating dtb directory in AnyKernel3..."
mkdir -p "$GITHUB_WORKSPACE/Positron_Kernel/anykernel/dtb"
log "➡️ Copying cust-atoll-ab.dtb..."
cp "$GITHUB_WORKSPACE/Positron_Kernel/out/arch/arm64/boot/dts/qcom/cust-atoll-ab.dtb" "$GITHUB_WORKSPACE/Positron_Kernel/anykernel/dtb"
...
cd "$GITHUB_WORKSPACE/Positron_Kernel/anykernel" || handle_error "Failed to enter AnyKernel3 directory"
zip -r9 "../$ZIP_NAME" ./* -x '*.git*' README.md ./*placeholder
cd "$GITHUB_WORKSPACE/Positron_Kernel" || handle_error "Failed to return to kernel directory"
...
tg_doc "$GITHUB_WORKSPACE/Positron_Kernel/$ZIP_NAME" "✅ Build finished after ${BUILD_DURATION_MINUTES} minutes ${BUILD_DURATION_SECONDS} seconds"

# --- Upload artifacts ---
ARTIFACT_DIR="$GITHUB_WORKSPACE/kernel_artifacts"
log "⬆️ Uploading artifacts to: $ARTIFACT_DIR"
mkdir -p "$ARTIFACT_DIR"
cp "$GITHUB_WORKSPACE/Positron_Kernel/out/arch/arm64/boot/Image.gz" "$ARTIFACT_DIR/"
cp "$GITHUB_WORKSPACE/Positron_Kernel/out/arch/arm64/boot/dtbo.img" "$ARTIFACT_DIR/"
cp "$GITHUB_WORKSPACE/Positron_Kernel/out/arch/arm64/boot/dts/qcom/cust-atoll-ab.dtb" "$ARTIFACT_DIR/"
cp "$GITHUB_WORKSPACE/Positron_Kernel/$ZIP_NAME" "$ARTIFACT_DIR/"

# --- Debugging output ---
log "🔍 Contents of $GITHUB_WORKSPACE/Positron_Kernel/out/arch/arm64/boot:"
ls -la "$GITHUB_WORKSPACE/Positron_Kernel/out/arch/arm64/boot/"
log "🔍 Contents of $GITHUB_WORKSPACE/Positron_Kernel/out/arch/arm64/boot/dts/qcom:"
ls -la "$GITHUB_WORKSPACE/Positron_Kernel/out/arch/arm64/boot/dts/qcom/"
