...
if [ -d "$GITHUB_WORKSPACE/Velion_Kernel" ]; then
...
if ! git clone "$KERNEL_SOURCE" -b "$KERNEL_BRANCH" "$GITHUB_WORKSPACE/Velion_Kernel" --depth=1; then
  handle_error "Failed to clone kernel source"
fi
cd "$GITHUB_WORKSPACE/Velion_Kernel" || handle_error "Failed to enter kernel directory"

# --- Integrate KernelSU-Next ---
log "🧩 Integrating KernelSU-Next..."
KERNELSU_DIR="$GITHUB_WORKSPACE/Velion_Kernel/kernel/KernelSU-Next"
KSU_REPO="https://github.com/KernelSU-Next/KernelSU-Next" # Perubahan URL

if [ ! -d "$KERNELSU_DIR" ]; then
  # Dapatkan tag terbaru
  LATEST_KSU_TAG=$(git ls-remote --tags --sort=v:refname "$KSU_REPO" | tail -n 1 | awk '{print $2}' | sed 's|refs/tags/||')

  if [ -z "$LATEST_KSU_TAG" ]; then
      handle_error "Failed to retrieve the latest tag for KernelSU-Next"
  fi

  git clone -b "$LATEST_KSU_TAG" "$KSU_REPO" "$KERNELSU_DIR" --depth=1 # Clone dengan tag terbaru
  log "✅ KernelSU-Next repository cloned (tag: $LATEST_KSU_TAG)."
else
  log "✅ KernelSU-Next directory already exists.  Updating..."
  cd "$KERNELSU_DIR"
  git fetch --all --tags
  LATEST_KSU_TAG=$(git describe --tags $(git rev-list --tags --max-count=1))
  git checkout "$LATEST_KSU_TAG"
  git pull origin "$LATEST_KSU_TAG"
  cd "$GITHUB_WORKSPACE/Velion_Kernel"
fi

# --- Determine driver directory ---
if [ -d "$GITHUB_WORKSPACE/Velion_Kernel/common/drivers" ]; then
  DRIVER_DIR="$GITHUB_WORKSPACE/Velion_Kernel/common/drivers"
elif [ -d "$GITHUB_WORKSPACE/Velion_Kernel/drivers" ]; then
  DRIVER_DIR="$GITHUB_WORKSPACE/Velion_Kernel/drivers"
else
...
export KBUILD_BUILD_HOST=VelionV1
...
if ! git clone -q -b Unoff https://github.com/AzyrRuthless/AnyKernel3 "$GITHUB_WORKSPACE/Velion_Kernel/anykernel"; then
  handle_error "Failed to clone AnyKernel3"
fi

# --- Copy files to AnyKernel3 ---
log "➡️ Copying Image.gz..."
cp "$GITHUB_WORKSPACE/Velion_Kernel/out/arch/arm64/boot/Image.gz" "$GITHUB_WORKSPACE/Velion_Kernel/anykernel"
log "➡️ Copying dtbo.img..."
cp "$GITHUB_WORKSPACE/Velion_Kernel/out/arch/arm64/boot/dtbo.img" "$GITHUB_WORKSPACE/Velion_Kernel/anykernel"
log "📁 Creating dtb directory in AnyKernel3..."
mkdir -p "$GITHUB_WORKSPACE/Velion_Kernel/anykernel/dtb"
log "➡️ Copying cust-atoll-ab.dtb..."
cp "$GITHUB_WORKSPACE/Velion_Kernel/out/arch/arm64/boot/dts/qcom/cust-atoll-ab.dtb" "$GITHUB_WORKSPACE/Velion_Kernel/anykernel/dtb"
...
cd "$GITHUB_WORKSPACE/Velion_Kernel/anykernel" || handle_error "Failed to enter AnyKernel3 directory"
zip -r9 "../$ZIP_NAME" ./* -x '*.git*' README.md ./*placeholder
cd "$GITHUB_WORKSPACE/Velion_Kernel" || handle_error "Failed to return to kernel directory"
...
tg_doc "$GITHUB_WORKSPACE/Velion_Kernel/$ZIP_NAME" "✅ Build finished after ${BUILD_DURATION_MINUTES} minutes ${BUILD_DURATION_SECONDS} seconds"

# --- Upload artifacts ---
ARTIFACT_DIR="$GITHUB_WORKSPACE/kernel_artifacts"
log "⬆️ Uploading artifacts to: $ARTIFACT_DIR"
mkdir -p "$ARTIFACT_DIR"
cp "$GITHUB_WORKSPACE/Velion_Kernel/out/arch/arm64/boot/Image.gz" "$ARTIFACT_DIR/"
cp "$GITHUB_WORKSPACE/Velion_Kernel/out/arch/arm64/boot/dtbo.img" "$ARTIFACT_DIR/"
cp "$GITHUB_WORKSPACE/Velion_Kernel/out/arch/arm64/boot/dts/qcom/cust-atoll-ab.dtb" "$ARTIFACT_DIR/"
cp "$GITHUB_WORKSPACE/Velion_Kernel/$ZIP_NAME" "$ARTIFACT_DIR/"

# --- Debugging output ---
log "🔍 Contents of $GITHUB_WORKSPACE/Velion_Kernel/out/arch/arm64/boot:"
ls -la "$GITHUB_WORKSPACE/Velion_Kernel/out/arch/arm64/boot/"
log "🔍 Contents of $GITHUB_WORKSPACE/Velion_Kernel/out/arch/arm64/boot/dts/qcom:"
ls -la "$GITHUB_WORKSPACE/Velion_Kernel/out/arch/arm64/boot/dts/qcom/"
