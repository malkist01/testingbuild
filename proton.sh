...
# Clone LeCmnGend's Clang
if [ ! -d "$HOME/leg-clang" ]; then
    if ! git clone https://gitlab.com/LeCmnGend/clang.git --depth=1 --branch clang-15 "$HOME/leg-clang"; then
      tg "❌ Failed to clone LeCmnGend Clang!"
      exit 1
    fi
fi
...
export PATH="$HOME/leg-clang/bin:$PATH"
export ARCH=arm64
