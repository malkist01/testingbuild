...
if [ -d "$GITHUB_WORKSPACE/Nitro_Kernel" ]; then
  log "🗑️ Cleaning up previous kernel directory..."
  rm -rf "$GITHUB_WORKSPACE/Nitro_Kernel"
fi

# --- Clone the kernel source ---
log "⬇️ Cloning kernel source from: $KERNEL_SOURCE (branch: $KERNEL_BRANCH)..."
if ! git clone "$KERNEL_SOURCE" -b "$KERNEL_BRANCH" "$GITHUB_WORKSPACE/Nitro_Kernel" --depth=1; then
  handle_error "Failed to clone kernel source"
fi
cd "$GITHUB_WORKSPACE/Nitro_Kernel" || handle_error "Failed to enter kernel directory"

# --- Integrate KernelSU-Next ---
log "🧩 Integrating KernelSU-Next..."
KERNELSU_DIR="$GITHUB_WORKSPACE/Nitro_Kernel/kernel/KernelSU-Next"
...
cd "$GITHUB_WORKSPACE/Nitro_Kernel"

# --- Determine driver directory ---
if [ -d "$GITHUB_WORKSPACE/Nitro_Kernel/common/drivers" ]; then
  DRIVER_DIR="$GITHUB_WORKSPACE/Nitro_Kernel/common/drivers"
elif [ -d "$GITHUB_WORKSPACE/Nitro_Kernel/drivers" ]; then
  DRIVER_DIR="$GITHUB_WORKSPACE/Nitro_Kernel/drivers"
else
...
if ! git clone -q https://github.com/AzyrRuthless/AnyKernel3 "$GITHUB_WORKSPACE/Nitro_Kernel/anykernel"; then
  handle_error "Failed to clone AnyKernel3"
fi

# --- Copy files to AnyKernel3 ---
log "➡️ Copying Image.gz..."
cp "$GITHUB_WORKSPACE/Nitro_Kernel/out/arch/arm64/boot/Image.gz" "$GITHUB_WORKSPACE/Nitro_Kernel/anykernel"
log "➡️ Copying dtbo.img..."
cp "$GITHUB_WORKSPACE/Nitro_Kernel/out/arch/arm64/boot/dtbo.img" "$GITHUB_WORKSPACE/Nitro_Kernel/anykernel"
log "📁 Creating dtb directory in AnyKernel3..."
mkdir -p "$GITHUB_WORKSPACE/Nitro_Kernel/anykernel/dtb"
log "➡️ Copying cust-atoll-ab.dtb..."
cp "$GITHUB_WORKSPACE/Nitro_Kernel/out/arch/arm64/boot/dts/qcom/cust-atoll-ab.dtb" "$GITHUB_WORKSPACE/Nitro_Kernel/anykernel/dtb"
...
cd "$GITHUB_WORKSPACE/Nitro_Kernel/anykernel" || handle_error "Failed to enter AnyKernel3 directory"
zip -r9 "../$ZIP_NAME" ./* -x '*.git*' README.md ./*placeholder
cd "$GITHUB_WORKSPACE/Nitro_Kernel" || handle_error "Failed to return to kernel directory"
...
tg_doc "$GITHUB_WORKSPACE/Nitro_Kernel/$ZIP_NAME" "✅ Build finished after ${BUILD_DURATION_MINUTES} minutes ${BUILD_DURATION_SECONDS} seconds"

# --- Upload artifacts ---
ARTIFACT_DIR="$GITHUB_WORKSPACE/kernel_artifacts"
log "⬆️ Uploading artifacts to: $ARTIFACT_DIR"
mkdir -p "$ARTIFACT_DIR"
cp "$GITHUB_WORKSPACE/Nitro_Kernel/out/arch/arm64/boot/Image.gz" "$ARTIFACT_DIR/"
cp "$GITHUB_WORKSPACE/Nitro_Kernel/out/arch/arm64/boot/dtbo.img" "$ARTIFACT_DIR/"
cp "$GITHUB_WORKSPACE/Nitro_Kernel/out/arch/arm64/boot/dts/qcom/cust-atoll-ab.dtb" "$ARTIFACT_DIR/"
cp "$GITHUB_WORKSPACE/Nitro_Kernel/$ZIP_NAME" "$ARTIFACT_DIR/"

# --- Debugging output ---
log "🔍 Contents of $GITHUB_WORKSPACE/Nitro_Kernel/out/arch/arm64/boot:"
ls -la "$GITHUB_WORKSPACE/Nitro_Kernel/out/arch/arm64/boot/"
log "🔍 Contents of $GITHUB_WORKSPACE/Nitro_Kernel/out/arch/arm64/boot/dts/qcom:"
ls -la "$GITHUB_WORKSPACE/Nitro_Kernel/out/arch/arm64/boot/dts/qcom/"
