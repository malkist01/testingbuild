...
# --- Integrate MKSU ---
log "🧩 Integrating MKSU..."
KERNELSU_DIR="$GITHUB_WORKSPACE/Kinesis_Kernel/kernel/MKSU"
if [ ! -d "$KERNELSU_DIR" ]; then
  git clone -b Azyr https://github.com/AzyrRuthless/KernelSU.git "$KERNELSU_DIR"
  log "✅ MKSU repository cloned."
fi
cd "$KERNELSU_DIR"
git stash && log "➖ Stashed current changes."
git checkout Azyr && log "➖ Switched to 'Azyr' branch."
git pull && log "🔄 MKSU repository updated."
