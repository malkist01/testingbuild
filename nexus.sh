...
if [ -d "$GITHUB_WORKSPACE/Nexus_Kernel" ]; then
    log "🗑️ Cleaning up previous kernel directory..."
    rm -rf "$GITHUB_WORKSPACE/Nexus_Kernel"
fi

# --- Clone the kernel source ---
log "⬇️ Cloning kernel source from: $KERNEL_SOURCE (branch: $KERNEL_BRANCH)..."
if ! git clone "$KERNEL_SOURCE" -b "$KERNEL_BRANCH" "$GITHUB_WORKSPACE/Nexus_Kernel" --depth=1; then
    handle_error "Failed to clone kernel source"
fi
cd "$GITHUB_WORKSPACE/Nexus_Kernel" || handle_error "Failed to enter kernel directory"

# --- Integrate KernelSU-Next ---
log "🧩 Integrating KernelSU-Next..."
KERNELSU_DIR="$GITHUB_WORKSPACE/Nexus_Kernel/kernel/KernelSU-Next"
...
cd "$GITHUB_WORKSPACE/Nexus_Kernel"

# --- Determine driver directory ---
if [ -d "$GITHUB_WORKSPACE/Nexus_Kernel/common/drivers" ]; then
    DRIVER_DIR="$GITHUB_WORKSPACE/Nexus_Kernel/common/drivers"
elif [ -d "$GITHUB_WORKSPACE/Nexus_Kernel/drivers" ]; then
    DRIVER_DIR="$GITHUB_WORKSPACE/Nexus_Kernel/drivers"
else
...
# --- Download and Extract Clang r547379 using aria2c ---
log "⬇️ Downloading Clang r547379 using aria2c..."
CLANG_DIR="$HOME/clang-r547379"
CLANG_ARCHIVE="$HOME/clang-r547379.tar.gz"
...
if [ ! -d "$CLANG_DIR" ]; then
    # Download using aria2c with multiple connections for faster download
    if ! aria2c -x8 -s8 -k1M -c -d "$HOME" -o "clang-r547379.tar.gz" "$CLANG_URL"; then
        handle_error "Failed to download Clang r547379 using aria2c"
    fi
    
    # Create directory and extract
    mkdir -p "$CLANG_DIR"
    log "📦 Extracting Clang r547379..."
    if ! tar -xzf "$CLANG_ARCHIVE" -C "$CLANG_DIR"; then
        handle_error "Failed to extract Clang r547379"
    fi
    
    # Clean up archive
    rm -f "$CLANG_ARCHIVE"
    log "✅ Clang r547379 downloaded and extracted to $CLANG_DIR"
else
    log "✅ Clang r547379 already exists at $CLANG_DIR"
fi
...
CLANG_VERSION=$($CLANG_DIR/bin/clang --version 2>&1 | head -n 1)
...
if ! git clone -q -b Ivory https://github.com/AzyrRuthless/AnyKernel3 "$GITHUB_WORKSPACE/Nexus_Kernel/anykernel"; then
    handle_error "Failed to clone AnyKernel3"
fi

# --- Copy files to AnyKernel3 ---
log "➡️ Copying Image.gz..."
cp "$GITHUB_WORKSPACE/Nexus_Kernel/out/arch/arm64/boot/Image.gz" "$GITHUB_WORKSPACE/Nexus_Kernel/anykernel"

log "➡️ Copying dtbo.img..."
cp "$GITHUB_WORKSPACE/Nexus_Kernel/out/arch/arm64/boot/dtbo.img" "$GITHUB_WORKSPACE/Nexus_Kernel/anykernel"

log "📁 Creating dtb directory in AnyKernel3..."
mkdir -p "$GITHUB_WORKSPACE/Nexus_Kernel/anykernel/dtb"

log "➡️ Copying cust-atoll-ab.dtb..."
cp "$GITHUB_WORKSPACE/Nexus_Kernel/out/arch/arm64/boot/dts/qcom/cust-atoll-ab.dtb" "$GITHUB_WORKSPACE/Nexus_Kernel/anykernel/dtb"
...
cd "$GITHUB_WORKSPACE/Nexus_Kernel/anykernel" || handle_error "Failed to enter AnyKernel3 directory"
zip -r9 "../$ZIP_NAME" ./* -x '*.git*' README.md ./*placeholder
cd "$GITHUB_WORKSPACE/Nexus_Kernel" || handle_error "Failed to return to kernel directory"
...
tg_doc "$GITHUB_WORKSPACE/Nexus_Kernel/$ZIP_NAME" "✅ Build finished after ${BUILD_DURATION_MINUTES} minutes ${BUILD_DURATION_SECONDS} seconds"
...
cp "$GITHUB_WORKSPACE/Nexus_Kernel/out/arch/arm64/boot/Image.gz" "$ARTIFACT_DIR/"
cp "$GITHUB_WORKSPACE/Nexus_Kernel/out/arch/arm64/boot/dtbo.img" "$ARTIFACT_DIR/"
cp "$GITHUB_WORKSPACE/Nexus_Kernel/out/arch/arm64/boot/dts/qcom/cust-atoll-ab.dtb" "$ARTIFACT_DIR/"
cp "$GITHUB_WORKSPACE/Nexus_Kernel/$ZIP_NAME" "$ARTIFACT_DIR/"

# --- Debugging output ---
log "🔍 Contents of $GITHUB_WORKSPACE/Nexus_Kernel/out/arch/arm64/boot:"
ls -la "$GITHUB_WORKSPACE/Nexus_Kernel/out/arch/arm64/boot/"

log "🔍 Contents of $GITHUB_WORKSPACE/Nexus_Kernel/out/arch/arm64/boot/dts/qcom:"
ls -la "$GITHUB_WORKSPACE/Nexus_Kernel/out/arch/arm64/boot/dts/qcom/"
